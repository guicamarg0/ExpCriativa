document.addEventListener("DOMContentLoaded", async () => {
    ...
    form.elements.data_nascimento.value = treinador.data_nascimento?.slice(0, 10) || "";
    form.elements.telefone.value = treinador.telefone || "";
    form.elements.cref.value = treinador.cref || "";
    form.elements.data_inicio.value = treinador.data_inicio?.slice(0, 10) || "";
    ...
    form.addEventListener("submit", async (event) => {
        ...
        if (resposta.status === "ok") {
            ...
            alert(resposta.mensagem || "Erro ao atualizar treinador.");
        }
    });
});